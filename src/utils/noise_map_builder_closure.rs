...
type SourceFunc = dyn Fn([f64; 3]) -> f64;

pub trait NoiseMapBuilderClosure {
    fn set_size(self, width: usize, height: usize) -> Self;

    fn set_source(self, source: Box<SourceFunc>) -> Self;
...
//     }
//
//     fn set_source(self, source_module: &'a dyn NoiseFn<f64, 3>) -> Self {
//         CylinderMapBuilder {
//             source_module,
//             ..self
//         }
...
//
//                 let value = self.source([point_x, current_height, point_z]);
//
//                 println!(
//                     "calculated value {} at {}, {}, {}",
...
pub struct PlaneMapBuilderClosure {
    is_seamless: bool,
    x_bounds: (f64, f64),
    y_bounds: (f64, f64),
    size: (usize, usize),
    source: Box<SourceFunc>
}

impl PlaneMapBuilderClosure {
    pub fn new(source: Box<SourceFunc>) -> Self {
        PlaneMapBuilderClosure {
            is_seamless: false,
            x_bounds: (-1.0, 1.0),
            y_bounds: (-1.0, 1.0),
            size: (100, 100),
...
impl NoiseMapBuilderClosure for PlaneMapBuilderClosure {
    fn set_size(self, width: usize, height: usize) -> Self {
        PlaneMapBuilderClosure {
            size: (width, height),
            ..self
        ...
        for y in 0..height {
            ...
            for x in 0..width {
                ...
                let final_value = if self.is_seamless {
                    let sw_value = (self.source)([current_x, current_y, 0.0]);
                    let se_value = (self.source)([current_x + x_extent, current_y, 0.0]);
                    let nw_value = (self.source)([current_x, current_y + y_extent, 0.0]);
                    let ne_value = (self.source)([current_x + x_extent, current_y + y_extent, 0.0]);
                    ...
                    (self.source)([current_x, current_y, 0.0])
                };
...
//
//                 result_map.set_value(x, y, self.source(point));
//             }
//         }
//
//         result_map
